...
<c:if test="${not empty sessionScope.message}">
    <div class="alert alert-success alert-dismissible fade show m-2 position-fixed top-0 end-0 shadow-lg"
         style="z-index: 9999;">
                ...
                <td class="text-center">
                    <div class="btn-group">
                        <button class="btn btn-outline-info btn-sm"
                                onclick="openEditCT('${ct.maCongThuc}', '${ct.tenCongThuc}')"><i
                                class="fas fa-flask"></i> Định lượng
                        </button>
                        <button class="btn btn-outline-light btn-sm"
                                onclick="renameCT('${ct.maCongThuc}', '${ct.tenCongThuc}')"><i class="fas fa-edit"></i>
                            Sửa
                        </button>
                        <button class="btn btn-outline-warning btn-sm" onclick="softDeleteCT('${ct.maCongThuc}')"><i
                                class="fas fa-eye-slash"></i> Ẩn
                        </button>
                        <button class="btn btn-outline-danger btn-sm" onclick="resetCT('${ct.maCongThuc}')"><i
                                class="fas fa-bomb"></i> Xóa sạch
                        </button>
                    </div>
                </td>
            </tr>
...
<div class="modal fade" id="modalAddNL">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="khonguyenlieu" method="post"><input type="hidden" name="action" value="add">
                <div class="modal-header border-0"><h5 class="modal-title">Thêm Nguyên Liệu</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <input type="text" name="ten" class="form-control mb-3" placeholder="Tên nguyên liệu" required>
                    <div class="row">
                        <div class="col-6"><input type="number" name="soLuong" class="form-control mb-3"
                                                  placeholder="Số lượng"></div>
                        <div class="col-6"><input type="text" name="donVi" class="form-control mb-3"
                                                  placeholder="Đơn vị"></div>
                    </div>
                    <select name="maLoai" class="form-select mb-3"><c:forEach var="l" items="${listLoai}">
...
<div class="modal fade" id="modalAddCT">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <form action="khonguyenlieu" method="post"><input type="hidden" name="action" value="addCT">
                <div class="modal-header border-0"><h5 class="modal-title text-info">Món Mới</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body"><input type="text" name="tenCongThuc" class="form-control"
                                               placeholder="Tên đồ uống..." required></div>
                <div class="modal-footer border-0">
                    <button type="submit" class="btn btn-info w-100">KHỞI TẠO</button>
                </div>
            </form>
        </div>
    </div>
</div>
<div class="modal fade" id="modalNhapKho">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <form action="khonguyenlieu" method="post"><input type="hidden" name="action" value="nhap"><input
                    type="hidden" name="id" id="nhap-id">
                <div class="modal-header border-0"><h6>Nhập thêm: <span id="nhap-ten"></span></h6></div>
                <div class="modal-body"><input type="number" name="soLuongNhap" class="form-control"
                                               placeholder="Số lượng..." required></div>
                <div class="modal-footer border-0">
                    <button type="submit" class="btn btn-success w-100">NHẬP KHO</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="modalEditCT" data-bs-backdrop="static">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header border-0"><h5 class="text-info">ĐỊNH LƯỢNG: <span id="label-ten-ct"></span></h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-5 border-end border-secondary">
                        ...
                        <select id="select-nl" class="form-select mb-3">
                            <c:forEach var="nl" items="${list}">
                                <option value="${nl.maNguyenLieu}">${nl.tenNguyenLieu} (${nl.donVi})</option>
                            </c:forEach>
                        </select>
                        <label class="small text-secondary mb-1">Số lượng sử dụng</label>
                        <div class="input-group"><input type="number" id="input-dl" class="form-control"
                                                        placeholder="0">
                            <button class="btn btn-info" onclick="addIngredient()"><i class="fas fa-plus"></i> Thêm
                            </button>
                        </div>
                    </div>
                    <div class="col-md-7">
                        <h6 class="text-success small fw-bold mb-3">DANH SÁCH THÀNH PHẦN</h6>
                        <div id="sub-list-container" class="custom-scroll"
...
<script>
    ...
    function deleteSub(idCTCT) {
        ...
        var p = new URLSearchParams();
        p.append('action', 'deleteCTCT');
        p.append('idCTCT', idCTCT);
        fetch('khonguyenlieu', {method: 'POST', body: p})
    ...
    function softDeleteCT(id) {
        if (confirm("Ẩn món này?")) submitForm('softDelete', {maCongThuc: id});
    }

    function resetCT(id) {
        if (confirm("Xóa sạch định lượng?")) submitForm('resetCT', {maCongThuc: id});
    }

    function submitForm(action, params) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = 'khonguyenlieu';
